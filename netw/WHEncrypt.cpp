...
//生成密文
bool CWHEncrypt::MD5Encrypt(const char* pszSourceData, char szMD5Result[LEN_MD5])
{
	//变量定义
	CMD5Aide MD5Aide;
	...
	//加密密文
	MD5Aide.MD5Update((unsigned char *)((const char*)pszSourceData), (unsigned int)(strlen(pszSourceData)*sizeof(char)));
	MD5Aide.MD5Final(szResult);

	//输出结果
	szMD5Result[0]=0;
	for (int i = 0; i < 16; i++) sprintf(&szMD5Result[i*2],("%02X"),szResult[i]);
		//sprintf_s(&szMD5Result[i * 2], LEN_MD5, "%02X", szResult[i]);

	return true;
}

////生成密文
//bool CWHEncrypt::XorEncrypt(const char* pszSourceData, char* pszEncrypData, unsigned short wMaxCount)
//{
//	//变量定义
//	CT2CW strSrcData(pszSourceData);
//	unsigned short szEncrypData[MAX_ENCRYPT_LEN+1]=L"";
//
//	//生成密钥
//	unsigned short wRandKey[ENCRYPT_KEY_LEN];
//	wRandKey[0]=strlenW(strSrcData);
//	for (unsigned short i=1;i<CountArray(wRandKey);i++) wRandKey[i]=rand()%0xFFFF;
//
//	//步骤准备
...
//
////解开密文
//bool CWHEncrypt::XorCrevasse(const char* pszEncrypData, char* pszSourceData, unsigned short wMaxCount)
//{
//	//设置结果
//	pszSourceData[0]=0;
//
...
//	unsigned short szSrcData[MAX_SOURCE_LEN]=L"";
//
//	//效验长度
//	unsigned short wEncrypPassLen=strlenW(strEncrypData);
//	if (wEncrypPassLen<ENCRYPT_KEY_LEN*8) return false;
//
//	//提取长度
//	unsigned short szTempBuffer[5]=L"";
//	szTempBuffer[CountArray(szTempBuffer)-1]=0;
//	CopyMemory(szTempBuffer,(LPCWSTR)strEncrypData,sizeof(unsigned short)*4);
//
//	//获取长度
//	unsigned short * pEnd=NULL;
//	unsigned short wSoureLength=(unsigned short)wcstol(szTempBuffer,&pEnd,16);
//
//	//长度效验
...
//	{
//		//获取密钥
//		unsigned short szKeyBuffer[5];
//		szKeyBuffer[CountArray(szKeyBuffer)-1]=0;
//		szTempBuffer[CountArray(szTempBuffer)-1]=0;
//		CopyMemory(szKeyBuffer,(LPCWSTR)(strEncrypData+i*8),sizeof(unsigned short)*4);
//		CopyMemory(szTempBuffer,(LPCWSTR)strEncrypData+i*8+4,sizeof(unsigned short)*4);
//
//		//提取密钥
//		unsigned short wKey=(unsigned short)wcstol(szKeyBuffer,&pEnd,16);
//		unsigned short wEncrypt=(unsigned short)wcstol(szTempBuffer,&pEnd,16);
//
//		//生成原文
//		szSrcData[i]=(unsigned short)((unsigned short)wKey^(unsigned short)wEncrypt);
//	}
//
//	//终止字符
...
//生成密文
bool CWHEncrypt::MapEncrypt(const char* pszSourceData, char* pszEncrypData, unsigned short wMaxCount)
{
	//效验参数
	assert(wMaxCount>strlen(pszEncrypData));
...
//解开密文
bool CWHEncrypt::MapCrevasse(const char* pszEncrypData, char* pszSourceData, unsigned short wMaxCount)
{
	//效验参数
	assert(wMaxCount>strlen(pszEncrypData));
	assert((pszEncrypData!=NULL)&&(pszSourceData!=NULL));
