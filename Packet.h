#ifndef PACKET_HEAD_FILE
#define PACKET_HEAD_FILE

#pragma pack(1)
#include "TCPPlatform.h"
#if TCP_TARGET_PLATFORM == TCP_PLATFORM_WIN32

#else
#ifndef SOCKET
#define SOCKET int
#endif

#ifndef SOCKET_ERROR
#define SOCKET_ERROR -1
#endif

#ifndef INVALID_SOCKET
#define INVALID_SOCKET -1
#endif

#ifndef INVALID_SOCKET
#define INVALID_SOCKET  (SOCKET)(~0)
#endif // !INVALID_SOCKET

#ifndef SOCKET_ERROR
#define SOCKET_ERROR            (-1)
#endif

#endif
//////////////////////////////////////////////////////////////////////////////////
//端口定义

#define MAX_CONTENT					512									//并发容量
#define PORT_AUTO_SELECT			INVALID_WORD						//自动端口

#ifdef _DEBUG
#define PORT_LOGON					1700								//登陆端口
#define PORT_CENTER					1710								//协调端口
#define PORT_MANAGER				1720								//管理端口
#define PORT_MSG_SERVER             1730                                //消息端口
#else
#define PORT_LOGON					1700								//登陆端口
#define PORT_CENTER					1710								//协调端口
#define PORT_MANAGER				1720								//管理端口
#define PORT_MSG_SERVER             1730                                //消息端口
#endif
//////////////////////////////////////////////////////////////////////////////////
//网络定义

//数据类型
#define DK_MAPPED					0x01								//映射类型
#define DK_ENCRYPT					0x02								//加密类型
#define DK_COMPRESS					0x04								//压缩类型

//长度定义
#define SOCKET_TCP_VER				0x66								//网络版本
#define SOCKET_TCP_BUFFER			16384								//网络缓冲
#define SOCKET_TCP_SEND_BUFFER		SOCKET_TCP_BUFFER					//网络缓冲
#define SOCKET_TCP_RCV_BUFFER		SOCKET_TCP_BUFFER * 10              //网络缓冲
#define SOCKET_TCP_PACKET			(SOCKET_TCP_BUFFER-sizeof(TCP_Head))//网络缓冲

//长度定义
#define SOCKET_UDP_BUFFER			16384								//网络缓冲
#define SOCKET_UDP_PACKET			(SOCKET_UDP_BUFFER-sizeof(UDP_Head))//网络缓冲

//////////////////////////////////////////////////////////////////////////////////
//结构定义

//网络内核
typedef struct TCP_Info
{
	unsigned char							cbDataKind;							//数据类型
	unsigned char							cbCheckCode;						//效验字段  //cbMod;//
	unsigned short							wPacketSize;						//数据大小
} TCP_Info;

//网络命令
typedef struct TCP_Command
{
	unsigned short							wMainCmdID;							//主命令码
	unsigned short							wSubCmdID;							//子命令码
} TCP_Command;

//网络包头
typedef struct TCP_Head
{
	TCP_Info						TCPInfo;							//基础结构
	TCP_Command						CommandInfo;						//命令信息
} TCP_Head;

//网络缓冲
typedef struct TCP_Buffer
{
	TCP_Head						Head;								//数据包头
	unsigned char							cbBuffer[SOCKET_TCP_PACKET];		//数据缓冲
} TCP_Buffer;

//////////////////////////////////////////////////////////////////////////////////

//网络内核
typedef struct UDP_Info
{
	unsigned char							cbDataKind;							//数据类型
	unsigned char							cbCheckCode;						//效验字段
	unsigned short							wPacketSize;						//数据大小
	unsigned short							wPacketIndex;						//数据序列
	unsigned short							wConnectIndex;						//连接索引
} UDP_Info;


//网络命令
typedef struct UDP_Command
{
	unsigned short							wMainCmdID;							//主命令码
	unsigned short							wSubCmdID;							//子命令码
} UDP_Command;


//网络包头
typedef struct UDP_Head
{
	UDP_Info						UDPInfo;							//基础结构
	UDP_Command						CommandInfo;						//命令信息
} UDP_Head;

//网络缓冲
typedef struct UDP_Buffer
{
	UDP_Head						Head;								//数据包头
	unsigned char							cbBuffer[SOCKET_UDP_PACKET];		//数据缓冲
} UDP_Buffer;


//////////////////////////////////////////////////////////////////////////////////

//内核命令
#define MDM_KN_COMMAND					0								//内核命令
#define SUB_KN_DETECT_SOCKET			1								//检测命令
#define SUB_KN_SHUT_DOWN_SOCKET			2								//中断网络
#define SUB_KN_VALIDATE_SOCKET			3								//验证命令
#define SUB_KN_VALIDATE_SOCKET_MOBILE   4								//验证命令  
//////////////////////////////////////////////////////////////////////////////////
//传输数据

#define IPC_VER						1									//版本标识
#define IPC_PACKET					(10240-sizeof(IPC_Head))			//最大包长
#define IPC_BUFFER					(sizeof(IPC_Head)+IPC_PACKET)		//缓冲长度

//数据包头
typedef struct IPC_Head
{
	unsigned short							wVersion;							//版本标识
	unsigned short							wPacketSize;						//数据大小
	unsigned short							wMainCmdID;							//主命令码
	unsigned short							wSubCmdID;							//子命令码
} IPC_Head;

//IPC 包结构
typedef struct IPC_Buffer
{
	IPC_Head						Head;								//数据包头
	unsigned char							cbBuffer[IPC_PACKET];				//数据缓冲
} IPC_Buffer;


//////////////////////////////////////////////////////////////////////////////////
//数据定义

//加密密钥
const unsigned long g_dwPacketKey=0xA55AA55A;

//发送映射
const unsigned char g_SendByteMap[256]=
{
	
};

//接收映射
const unsigned char g_RecvByteMap[256]=
{
	
};

//////////////////////////////////////////////////////////////////////////////////

#pragma pack()

#endif